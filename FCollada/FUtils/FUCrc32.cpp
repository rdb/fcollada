...
// #define GENERATE_CRC32_TABLE
#ifdef GENERATE_CRC32_TABLE
	// ** The above table was created using the code below.
	// ** The code below was adapted from http://www.createwindow.com/programming/crc32/

	uint32 Reflect(unsigned long ref, char ch)
	{
		uint32 value(0);
	...
	FCOLLADA_EXPORT void InitCRCTable()
	{
		uint32 polynomial = 0x04c11db7;
		uint32 crcTable[256];

		ofstream fileOut;
		fileOut.open("CRCTable.txt", ios_base::out | ios_base::binary);
		...
		for (long i = 0; i <= 0xFF; ++i)
		{
			...
			char sz[256];
			snprintf(sz, 256, "0x%08X, ", crcTable[i]);
			fileOut << sz;
		}
